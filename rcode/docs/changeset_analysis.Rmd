...
dat = rbind(philippines, haiti)dat
```
...
### Getting an idea for when the edits occur
```{r when_edits_occured}
haiti_users_by_day = c(by(haiti$user, haiti$DaysSinceEvent, unique))
phil_users_by_day = c(by(philippines$user, philippines$DaysSinceEvent, unique))


n <- max(length(haiti_users_by_day), length(phil_users_by_day))
length(haiti_users_by_day) <- n                    
length(phil_users_by_day) <- n

height <- rbind(haiti_users_by_day, phil_users_by_day)

barplot(height, beside = TRUE, main="Users editing each day since event", xlab="Days Since Event", ylab="Total number of users editing")
